...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupListeners() {
        ...
        // 💰 치트: 상점 버튼 롱클릭하면 1000원 충전!
        binding.shopNav.setOnLongClickListener(v -> {
            GameState state = viewModel.getState().getValue();
            if (state != null) {
                state.setCoin(state.getCoin() + 1000);
                Snackbar.make(binding.getRoot(), "💰 치트 발동! 코인 +1000", Snackbar.LENGTH_SHORT).show();
            }
            return true;
        });
